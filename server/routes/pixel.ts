...
export const updatePixel: RequestHandler = async (req, res) => {
  try {
    ...
    res.json({
      ...
      createdAt:
        updatedPixel.created_at?.toISOString() || new Date().toISOString(),
  ...
  try {
    ...
    events.forEach((event: any) => {
      ...
      eventsByType[event.event_type] =
        (eventsByType[event.event_type] || 0) + event.count;
...
export const getPixelEvents: RequestHandler = async (req, res) => {
  try {
    ...
    // Get total count
    const countSQL =
      "SELECT COUNT(*) as total FROM pixel_events WHERE pixel_id = ?";
