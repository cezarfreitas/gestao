  ...
  try {
    ...
    if (search) {
      whereConditions.push(
        "(l.nome LIKE ? OR l.whatsapp LIKE ? OR l.cep LIKE ?)",
      );
    ...
    const transformedLeads: Lead[] = leads.map((row: any) => ({
      ...
      interaction: {
        sessionStartTime:
          row.session_start_time?.toISOString() || new Date().toISOString(),
        timeOnSite: row.time_on_site || 0,
        currentTimestamp:
          row.interaction_timestamp?.toISOString() || new Date().toISOString(),
